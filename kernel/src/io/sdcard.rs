...
lazy_static! {
    static ref PERIPHERALS: SpinLock<Peripherals> = SpinLock::new(Peripherals::take().unwrap());
}
...
fn init_sdcard() -> SDCard<SPIImpl<SPI0>> {
    ...
    info_sync!("init sdcard!");
    sd
}

static mut sdcard_new: bool = false;

pub fn new_sdcard()->SDCard<SPIImpl<SPI0>>{
    unsafe {
        if sdcard_new {
            panic!("sdcard have been new before");
        }
        else {
            unsafe { sdcard_new = true; }
            init_sdcard()
        }
    }
}

pub struct SDCardDev{
    inner:SpinLock<SDCard<SPIImpl<SPI0>>>
}

impl BlockRead for SDCardDev {
    fn read_block(&self, blk_no: usize, buf: &mut [u8]) {
        self.inner.lock().unwrap().read_sector(buf,blk_no as u32).unwrap();
    }
}

impl BlockWrite for SDCardDev {
    fn write_block(&self, blk_no: usize, buf: &[u8]) {
        self.inner.lock().unwrap().write_sector(buf,blk_no as u32).unwrap();
    }
}

impl BlockReadWrite for SDCardDev {}

impl SDCardDev {
    pub fn new()->Self{
        Self{
            inner: SpinLock::new(new_sdcard())
        }
    }
}
